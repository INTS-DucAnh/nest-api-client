...
export default function SelectMutipleCategory({ onChange }: { onChange: (id: string) => void }) {
  const [size, SetSize] = useState<number>(10);
  const [page, SetPage] = useState<number>(1);
  const [total, SetTotal] = useState<number>(10);
  const [key, SetKey] = useState<string>('');
  const [listSelect, SetListSelect] = useState<CategoryFindItemType[]>([]);
  const [list, SetList] = useState<CategoryFindItemType[]>([]);
  ...
  const handleSelect = (item: CategoryFindItemType) => {
    onChange(item.id);
    SetListSelect(list => (list.findIndex(e => e.id === item.id) !== -1 ? list.filter(e => e.id !== item.id) : [...list, item]));
  };
