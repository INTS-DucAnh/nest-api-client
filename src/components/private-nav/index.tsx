  ...
  return (
    <aside className='h-full w-fit'>
      <Card className='h-full w-72 rounded-none flex flex-col justify-start items-start'>
        <Link className='h-fit w-full box-border p-4 px-7 flex gap-3 items-center' to={PATH_BASE.user}>
        ...
        <div className='flex-1 flex flex-col w-full'>
          <ScrollArea className='flex-1'>
            <div className='h-full w-full flex flex-col gap-3 box-border p-2 px-3'>
              {Object.entries(PRIVATE_NAV_COMPO).map(([target, comp]: [target: string, comp: NavigationItem]) => {
